...
This repo is a base starter template (Next.js + Netlify/Cloudflare Workers blog).
...
```
You are restyling this Next.js blog starter template to match a new visual design.
This is a DESIGN-ONLY task. Do not change configuration, data, content, routing,
or build/deploy setup — only how things look.

## Never touch (config, data, infra, logic)
- config.json                          (site metadata — title, description, base_url, etc.)
...
- content/**                           (blog post markdown/data)
...
- public/styles/global.css and public/styles/content.module.css (colors, type, spacing, tokens)
...
- Static assets in public/ (images, icons, fonts) if the design calls for them
...
## Rules
1. Preserve every existing feature: pagination, tag filtering, post list/detail
   pages, SEO tags, RSS/sitemap if present, and all current routes/URLs.
2. Do not introduce new npm packages (no Tailwind, no UI kit, no icon library)
   unless I explicitly ask for one by name.
3. Do not change any text/copy from config.json or content/** unless the design
   input explicitly includes new copy.
...
Apply this design across all pages/components consistently (home, post list,
post detail, tags, navigation, footer) and summarize what you changed and why
at the end.
```
